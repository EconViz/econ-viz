...
PATH = PricePath(MODEL, budget=LinearBudget(px=2.0, py=2.0, income=40.0), price="px", price_range=(0.8, 6.0), n=5)
...
CASES = [
    ...
    (
        "compensated_budget",
        lambda: Canvas(x_max=20, y_max=15).add_decomposition(
            DEC, compensated_budget_color=COLOR, compensated_budget_linewidth=WIDTH, compensated_budget_linestyle="-"
        ),
        lambda: Canvas(x_max=20, y_max=15).add_decomposition(
    ...
    (
        "substitution",
        lambda: Canvas(x_max=20, y_max=15).add_decomposition(
            DEC, substitution_color=COLOR, effect_arrow_linewidth=WIDTH
        ),
